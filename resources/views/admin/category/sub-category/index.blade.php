...
@section('content')
    <nav class="page-breadcrumb flex justify-between">
        ...
        <button type="button" class="btn btn-outline-primary btn-icon-text mb-2 mb-md-0" data-toggle="modal"
            data-target="#addMainCategoryModal"> Add Sub Category</button>
                            ...
                            <tbody>
                              @foreach ($subCategories as $subCategory )
                                <tr>
                                    ...
                                    {{-- {{ asset('storage/' . $mainCategroy->icon) }} --}}
                                    <td>
                                        <img src="{{ asset('storage/' . $subCategory->icon) }} " alt="Icon" srcset=""
                                            style="max-width: 200px; height: auto;">
                                    </td>
                                    <td>
                                        <img src="{{ asset('storage/' . $subCategory->image) }}" alt="Image" srcset=""
                                            style="max-width: 200px; height: auto;">
                                    </td>
                                    <td>{{$subCategory->status}}</td>
                                    <td>
                                        <div class="flex gap-2">
                                            <a href="{{ route('admin.Editsubcategory', $subCategory->id) }}" class="btn"><i class="me-1 icon-md" data-feather="edit"
                                                        style="color: green"></i></a>
                                            <button id="deleteButton" class="btn" data-toggle="modal"
                                                    data-target="#deleteModal" data-id="{{ $subCategory->id }}">
    ...
    <div class="modal fade" id="addMainCategoryModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('subcategory.store') }}" method="POST" enctype="multipart/form-data" id="addCategoryForm">
                    @csrf
                    <div class="modal-body">

                        <div class="form-group">
                            <label for="categoryName">Sub Category Name:</label>
                            <input type="text" name="Sub_category_name" class="form-control inputbgtextcolor" id="categoryName" required>
                        </div>
                        ...
                        <div class="form-group mt-2">
                            ...
                            <div class="flex justify-start">
                                <span id="uploadIcon" class="cursor-pointer font-mono text-xl" role="button" aria-label="Upload Icon">
                                ...
                                <img id="iconPreview" src="" alt="Icon Preview" style="display: none; margin-top: 10px; max-width: 350px; height: 100px;" />
                            </div>
                            <input hidden type="file" name="icon" class="form-control inputbgtextcolor" id="categoryIcon" accept="image/png, image/jpeg, image/jpg, image/gif" required onchange="onFileChangeIcon(event)">
                        </div>

                        <div class="form-group mt-2">
                            <label for="categoryImageInput">Image:</label>
                            <div class="flex justify-start">
                                <span id="uploadImage" class="cursor-pointer font-mono text-xl" role="button" aria-label="Upload Image">
                                    <svg viewBox="0 0 1024 1024" class="w-20" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" fill="#D74825">
                                    <path
                                        d="M736.68 435.86a173.773 173.773 0 0 1 172.042 172.038c0.578 44.907-18.093 87.822-48.461 119.698-32.761 34.387-76.991 51.744-123.581 52.343-68.202 0.876-68.284 106.718 0 105.841 152.654-1.964 275.918-125.229 277.883-277.883 1.964-152.664-128.188-275.956-277.883-277.879-68.284-0.878-68.202 104.965 0 105.842z"
                                ...
                                <img id="imagePreview" src="" alt="Image Preview" style="display: none; margin-top: 10px; max-width: 350px; height: 100px;" />
                            </div>
                            <input hidden type="file" name="image" class="form-control inputbgtextcolor" id="categoryImageInput" accept="image/png, image/jpeg, image/jpg, image/gif" required onchange="onFileChange(event)">
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '#deleteButton', function() {
            ...
            const formAction = `{{ route('subcategory.delete', '') }}/${categoryId}`;
