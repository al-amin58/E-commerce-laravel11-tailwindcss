...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Color List</h1>
        </div>
                                ...
                                <table id="tasks-table" class="table text-nowrap mb-0 table-bordered border-top border-bottom">
                                    ...
                                    @foreach ($colors as $color)
                                        <tr>
                                            ...
                                            <td class="text-muted fs-15 fw-semibold">{{ $color->color }}</td>
                                            <td>
                                                <div style="background-color: {{ $color->color_code }}; padding: 10px; margin: 5px;">
                                                    <strong>{{ $color->color }}</strong> ({{ $color->color_code }})
                                                </div>
                                            </td>
                                            <td class="{{ $color->status === 'inactive' ? 'text-danger' : 'text-primary' }} fs-15 fw-semibold">{{ $color->status }}</td>
                                            <td>
                                                <div class="d-flex align-items-stretch justify-content-center">
                                                    <a href="{{ route('color.Edit', $color->id) }}" class="btn btn-sm btn-outline-primary border me-2"  data-bs-original-title="Edit">
                                                        <i class="fe fe-edit-2"></i>
                                                    </a>
                                                    <button id="deleteButton" data-id="{{ $color->id }}" data-toggle="modal" data-target="#deleteModal" class="btn btn-sm btn-outline-secondary border "  data-bs-original-title="Delete">
                ...
                <form action="{{ route('color.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group mt-2">
                            ...
                            <div class="input-group">
                                <input type="text" name="color_code" class="form-control" placeholder="Enter Color Code (e.g., #FFFFFF)" required>
                            </div>
                        </div>
                        <div class="form-group mt-2">
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '#deleteButton', function() {
            ...
            const formAction = `{{ route('color.delete', '') }}/${categoryId}`;
