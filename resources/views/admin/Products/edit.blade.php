                    ...
                    <div class="example">
                        <form  action="{{route('product.update', $product->id)}}" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="col-md-12 col-xl-3">
                                    <div class="form-group">
                                        <label class="form-label">Price: <sup><span
                                                    class="text-danger fw-700 fs-6 ">*</span></sup></label>
                                        <input type="number" name="price" value="{{$product->price}}" class="form-control" placeholder="Enter Price" required>
                                    </div>
                                </div>
                                <div class="col-md-12 col-xl-3">
                                    <div class="form-group">
                                        <label class="form-label">Discount
                                            Price: <sup><span
                                                    class="text-danger fw-700 fs-6 ">%</span></sup></label>
                                        <input type="number" name="discount_percentage" value="{{$product->discount_percentage}}" placeholder="Enter Discount Price %" class="form-control" min="0" max="100">
                                    </div>
                                </div>
                                <div class="col-md-12 col-xl-3">
                                    <div class="form-group">
                                ...
                                <div class="col-md-12 col-xl-3">
                                    <div class="form-group">
                                        <label class="form-label">SKU: <sup><span
                                                    class="text-danger fw-700 fs-6 ">*</span></sup></label>
                                        <input type="text" class="form-control" name="sku" value="{{$product->sku}}"  placeholder="Enter SKU"  required>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12 col-xl-4">
                                    <div class="form-group">
                                        ...
                                        <select name="main_category" class="form-control select2-show-search form-select"  data-placeholder="Choose one">
                                            <option value="{{$product->main_category}}">{{$product->mainCategory->main_category_name}}</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="sub_category" class="form-control select2-show-search form-select" data-placeholder="Choose one">
                                            ...
                                            @foreach ($subcategories as $subcategory)
                                                <option value="{{ $subcategory->id }}" {{ $product->sub_category == $subcategory->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="short_description" id="shortDescription" >{{$product->short_description}}</textarea>
                            </div>
                            <!--End Short Editor-->
                            <!-- Full Editor -->
                            <div class="form-group">
                                ...
                                <textarea name="full_description" id="fullDescription" >{{$product->full_description}}</textarea>
                            </div>
                            <!--End Full Editor-->
                            <!-- Thumbnail Image -->
                            <div class="form-group">
                                ...
                                <span id="uploadthumbnailImage" class="cursor-pointer font-mono text-xl" role="button"
                                    ...
                                    <img id="thumbnailPreview" src="{{ asset('images/products/' . $product->thumbnail_image) }}" alt="thumbnail Preview"
                                         style="display: {{ $product->thumbnail_image ? 'block' : 'none' }}; margin-top: 10px; max-width: 1000px; height: 100px;" />
                                </div>
                                    ...
                                    <div class="flex">
                                        <div class="d-flex flex-wrap justify-content-start" >
                                            @foreach ($product->images as $image)
                                                <div class="d-flex justify-content-center position-relative" data-image="{{ $image->image_id }}" style="width: 150px; overflow: hidden; margin: 5px; border-radius: 6px; height: 80px;">
                                                    <span class="remove-image" style="position: absolute; top: -8px; right: 8px; cursor: pointer; color: red; font-size: 28px; font-weight: normal;" data-image-id="{{ $image->id }}">&times;</span>
                                                    <img src="{{ asset($image->image_id) }}" alt="Image Preview" style="width: auto; object-fit: cover; height: 100%;" />
                                                </div>
                                            @endforeach
                                        </div>
                            ...
                            <div class="modal-footer">
                                <button type="submit" id="saveButton" class="btn btn-primary p-3 fw-semibold">Update Product</button>
                            </div>
                        </form>
                    </div>
                </div>
...
{{--    singal image delete--}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Add event listeners to all remove image spans
            const removeImageButtons = document.querySelectorAll('.remove-image');

            removeImageButtons.forEach(button => {
                button.addEventListener('click', function() {
                    const imageContainer = this.parentElement; // Get the parent container of the image
                    const imageId = this.getAttribute('data-image-id'); // Get the image ID from the data attribute

                    // Optionally, send an AJAX request to delete the image from the database

                        fetch(`http://localhost/E-commerce-laravel11-tailwindcss/admin/delete-image/${imageId}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}', // Make sure this is rendered correctly in Blade
